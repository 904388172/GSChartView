...
@protocol GSXAxisViewDelegate <NSObject>

@optional
- (void)xAxisView:(GSXAxisView *)xAxisView didClickButtonAtIndex:(NSInteger)index;

@end

@interface GSXAxisView : UIView

@property (nonatomic, weak) id<GSXAxisViewDelegate> delegate;
...
/**
 *  点是否允许点击当前X轴上显示对应的数值，其余不显示
 */
@property (assign, nonatomic, getter=isShowThePointNum) BOOL showThePointNum;
/**
 *  是否显示所有点的数据
 */
@property (assign, nonatomic, getter=isShowAllLabel) BOOL showAllLabel;
